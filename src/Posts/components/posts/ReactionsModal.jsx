...
import { FiX, FiChevronDown, FiChevronUp, FiHeart, FiThumbsUp, FiSmile, FiMeh, FiFrown, FiZap } from 'react-icons/fi';
import { Link } from 'react-router-dom';

const ReactionsModal = ({ postId, onClose, darkMode = false }) => {
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* Modal Header */}
          <div className={`p-5 ${darkMode ? 'border-b border-gray-800' : 'border-b border-gray-100'}`}>
          ...
          <div className="overflow-y-auto flex-1 p-4">
            {isLoading ? (
              ...
              <motion.div 
                ...
                animate="visible"
              >
                {(activeTab === 'all' ? allReactors : reactions?.[activeTab] || []).map((user, i) => (
                  <motion.div 
                    key={user.id} 
                      ...
                      {user.comment && (
                        ...
                        >
                          {expandedUsers[user.id] ? (
                            <FiChevronUp className={darkMode ? 'text-gray-300' : 'text-gray-500'} />
                          ) : (
                            <FiChevronDown className={darkMode ? 'text-gray-300' : 'text-gray-500'} />
                          )}
                        </motion.button>
                      )}
                    </div>
                      ...
                      {expandedUsers[user.id] && user.comment && (
                        ...
                        >
                          <div className={`p-3 rounded-lg ${
                            ...
                            <p className="text-sm whitespace-pre-line">{user.comment}</p>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
